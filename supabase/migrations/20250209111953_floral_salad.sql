...
BEGIN
  ...
  BEGIN
    v_user_id := CASE 
      WHEN user_id LIKE 'did:privy:%' THEN 
        COALESCE(
          -- Try uuid_generate_v5 first
          uuid_generate_v5(uuid_generate_v4(), user_id),
          -- Fallback to gen_random_uuid
          uuid_generate_v5(gen_random_uuid(), user_id),
          -- Final fallback using md5
          uuid_generate_v5(uuid_nil(), md5(user_id))
        )
      ELSE
        CASE 
  ...
  EXCEPTION WHEN OTHERS THEN
    -- Ultimate fallback using md5
    v_user_id := uuid_generate_v5(uuid_nil(), md5(user_id));
  END;
