...
DROP POLICY IF EXISTS "authenticated_users_access" ON users;
...
-- Update the sync_user_profile function to be more permissive
...
BEGIN
  ...
  EXCEPTION WHEN OTHERS THEN
    -- Fallback to using the ID directly if conversion fails
    user_id := NEW.id::uuid;
  END;
  ...
  EXCEPTION WHEN OTHERS THEN
    ...
    RAISE WARNING 'Error in sync_user_profile insert/update: %', SQLERRM;
  END;
