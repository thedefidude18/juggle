...
BEGIN
  ...
  BEGIN
    v_uuid := CASE 
      ...
      ELSE
        CASE 
          WHEN input_id ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$' 
          THEN input_id::uuid
          ELSE uuid_generate_v5(uuid_generate_v4(), input_id)
        END
    END;
  EXCEPTION WHEN OTHERS THEN
    v_uuid := uuid_generate_v5(uuid_generate_v4(), input_id);
  END;
  ...
  RETURN v_wallet_id;
EXCEPTION WHEN OTHERS THEN
  RAISE WARNING 'Error in sync_user_and_wallet_v2: %', SQLERRM;
  RETURN NULL;
END;
$$;
