...
-- Create single function to initialize wallet
...
CREATE OR REPLACE FUNCTION sync_user_and_wallet_v2(p_input_id text)
RETURNS bigint
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
  ...
  BEGIN
    v_user_id := CASE 
      WHEN p_input_id LIKE 'did:privy:%' THEN 
        uuid_generate_v5(uuid_generate_v4(), p_input_id)
      ELSE
        p_input_id::uuid
    END;
  EXCEPTION WHEN OTHERS THEN
    v_user_id := uuid_generate_v5(uuid_generate_v4(), p_input_id);
  END;
